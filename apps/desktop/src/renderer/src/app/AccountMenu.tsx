...
import "./account-menu-trigger.scss";
import {
  ChartBar,
  Check,
  ChevronDown,
...
export function AccountMenu({
  ...
  return (
    <div className="rm-account-menu-root" ref={rootRef}>
      {open && (
        <section className="rm-account-menu" role="dialog" aria-label={tr("account.menu.title")}>
          <div className="rm-account-menu-profile">
            ...
            {account && (
              <button
                className="rm-account-profile-settings"
                type="button"
                aria-label={tr("account.center.open")}
                title={tr("account.center.open")}
                onClick={() => {
                  setOpen(false);
                  onOpenAccountCenter();
                }}
          ...
          {account && (
            <div className="rm-account-overview">
              <div className="rm-account-overview-row">
                <span>{account.membership?.planName ?? tr("ui.c152cd8ac13b")}</span>
                <button
                  type="button"
                  onClick={() => {
                    setOpen(false);
              ...
              <div className={`rm-credit-section ${creditsOpen ? "open" : ""}`}>
                ...
                {creditsOpen && (
                  <div className="rm-credit-actions" role="menu" aria-label={tr("account.credits.actions")}>
                    ...
                    >
                      <CreditCard size={14} />
                      <span>{tr("account.credits.topUp")}</span>
                    ...
                    >
                      <ChartBar size={14} />
                      <span>{tr("account.credits.usage")}</span>
                      <ExternalLink size={12} />
                    </button>
                  </div>
                )}
            ...
            <div className="rm-account-menu-section rm-space-section">
              <div className="rm-current-space-row">
                ...
                <div className="rm-current-space">
                  ...
                  {spaces.length > 1 && (
                    <button
                      className="rm-space-switch"
                      type="button"
                      aria-label={tr("account.space.switch")}
                      title={tr("account.space.switch")}
              ...
              {spacesOpen && spaces.length > 1 && (
                <div className="rm-space-list" role="listbox" aria-label={tr("ui.8ed04ed0151c")}>
                  {spaces.map((space) => {
                    const active = space.id === activeSpace?.id;
                    return (
                      <button
                        key={space.id}
                        className={`rm-space-select ${active ? "active" : ""}`}
                        type="button"
                        role="option"
                        aria-selected={active}
                        disabled={busy}
                        onClick={() => {
                          void onSwitchSpace(space.id).then((switched) => {
                            if (switched) setSpacesOpen(false);
                          });
                        }}
                      >
          ...
          <button
            ...
            type="button"
            disabled={busy}
            onClick={() => (account || canClearAuth ? onSignOut() : onSignIn())}
          >
            {busy ? <LoaderCircle className="spin" size={14} /> : account ? <LogOut size={14} /> : <LogIn size={14} />}
            <span>
              {account ? tr("ui.094774b4a77b") : canClearAuth ? tr("ui.9940d7d9eaa6") : tr("ui.bc23dccbf868")}
            </span>
          </button>
        </section>
      )}
        ...
        >
          ...
          {expanded && (
            <span className="rm-rail-account-copy">
              <strong>{account?.displayName ?? tr("ui.6b882773fd47")}</strong>
              {!account && <small>{tr("ui.f1519207c301")}</small>}
            </span>
          )}
        </button>
      </div>
...
function AccountAvatar({ displayName, avatarUrl }: { displayName?: string; avatarUrl?: string | null }) {
  return (
    <div className="rm-account-menu-avatar">
      {avatarUrl ? (
