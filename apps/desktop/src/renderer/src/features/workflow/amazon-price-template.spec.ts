...
describe("Amazon price workflow template", () => {
  it("creates a configured five-step browser-to-XLSX-and-email workflow", () => {
    let sequence = 0;
    const draft = createAmazonPriceWorkflowDraft({
      localProjectId: "project_1",
      url: "https://www.amazon.com/dp/B000TEST#reviews",
      workbookPath: "outputs/amazon/price.xlsx",
      screenshotsDirectory: "outputs/amazon/screenshots",
    ...
    expect(draft.nodes.map((node) => node.executorKey)).toEqual(AMAZON_PRICE_WORKFLOW_KEYS);
    ...
    expect(draft.nodes[3]?.config).toEqual({
      workbookPath: "outputs/amazon/price.xlsx",
      sheetName: "Price History",
    });
    expect(draft.nodes[4]?.config).toEqual({
      recipient: "lizhug.steven@gmail.com",
    });
  ...
  it("rejects non-Amazon and insecure URLs", () => {
    expect(() => normalizeAmazonUrl("http://amazon.com/dp/test")).toThrow("HTTPS");
    expect(() => normalizeAmazonUrl("https://example.com/product")).toThrow("Amazon");
  });
});
