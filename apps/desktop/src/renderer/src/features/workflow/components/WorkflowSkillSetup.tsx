...
export function WorkflowSkillSetup({
  model,
  actions
}: {
  ...
  const [selectedSkillId, setSelectedSkillId] = useState(
    () => availableSkills[0]?.id ?? WORKFLOW_SKILLS[0]?.id ?? ""
  );
  const skill =
    availableSkills.find((candidate) => candidate.id === selectedSkillId) ??
    availableSkills[0] ??
    null;
  const [valuesBySkill, setValuesBySkill] = useState<Record<string, Record<string, string>>>(
    () => Object.fromEntries(
      WORKFLOW_SKILLS.map((candidate) => [
        candidate.id,
        Object.fromEntries(
          candidate.setupFields.map((field) => [field.key, field.defaultValue ?? ""])
        )
      ])
    )
  );
  ...
  async function chooseDirectory(key: string) {
    ...
    if (directory) setValue(key, directory);
  }

  const complete = skill.setupFields.every(
  ...
  return (
    <section className="workflow-amazon-setup">
      ...
      {skill.setupFields.map((field) =>
        field.kind === "directory" ? (
          <button
            key={field.key}
            type="button"
            title={values[field.key] || `选择${field.label}`}
            onClick={() => void chooseDirectory(field.key)}
          >
            <FolderOpen size={13} />
            {values[field.key] ? `已选择${field.label}` : `选择${field.label}`}
          </button>
        ) : (
          <label key={field.key}>
      ...
      <button
        ...
        type="button"
        disabled={!model.selectedProjectId || !complete}
        onClick={() =>
