...
export function workflowSkillById(skillId: string): WorkflowSkillDefinition {
    ...
    if (!skill)
        throw new Error(tr("ui.151c949223c2"));
    return skill;
}
export function availableWorkflowSkills(definitions: DesktopWorkflowNodeDefinition[]): WorkflowSkillDefinition[] {
    return workflowSkills().filter((skill) => skill.requiredExecutorKeys.every((executorKey) => definitions.some((definition) => definition.executorKey === executorKey && definition.available)));
}
