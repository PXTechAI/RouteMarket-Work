...
import type { AgentLocalToolGroup, ChatModel, ProjectChatEvent, ProjectContext, ProjectSummary, RouteMarketWorkApi, WorkState } from "../../../../shared/desktop-api";
...
export function useAgentWorkspace({ api, active, authStatus, selectedProject, projectContext, models, modelsLoading, onChooseProject }: AgentWorkspaceOptions): {
    ...
    const [localToolGroups, setLocalToolGroups] = useState<AgentLocalToolGroup[]>(defaultLocalToolGroups);
    ...
    const selectedModel = useMemo(() => models.find((model) => model.code === selectedModelCode) ?? null, [models, selectedModelCode]);
    const selectionScope = selectedProject?.localProjectId ?? "general";
    const conversationKey = selectedAgent
        ? `${selectionScope}:${selectedAgent.id}`
        : "";
    ...
    useEffect(() => {
        ...
        setSelectedModelCode((current) => {
            ...
            if (models.some((model) => model.code === current))
                return current;
    ...
    useEffect(() => {
        return api.onProjectChatEvent((event) => {
            ...
            if (event.type === "artifacts") {
                setMessagesByConversation((current) => updateAssistantMessage(current, key, event.requestId, (message) => ({
                    ...message,
                    artifacts: [...new Map([...(message.artifacts ?? []), ...event.artifacts].map((artifact) => [artifact.relativePath, artifact])).values()]
                })));
                return;
            }
    ...
    const send = useCallback(async () => {
        ...
        const sessionId = sessionIdsRef.current.get(key) ??
        ...
        try {
            await api.sendProjectMessage({
                ...
                preferredChatProtocol: selectedModel?.preferredChatProtocol ?? null,
                ...(selectedModel?.preferredChatProtocol === "openai_responses" && selectedModel.supportsReasoningSummary
            ...
            const messageText = nextError instanceof Error ? nextError.message : tr("ui.b3a5f8407da2");
            setError(messageText);
            setMessagesByConversation((current) => updateAssistantMessage(current, key, requestId, (assistant) => ({
                ...assistant,
    ...
    const actions = useMemo<AgentPageActions>(() => ({
        ...
        onSelectAgent(agentId) {
            if (!activeRequestRef.current) {
                selectedAgentsByProjectRef.current.set(selectionScope, agentId);
        ...
        onToolGroupChange(group, enabled) {
            setLocalToolGroups((current) => enabled
    ...
    }), [onChooseProject, refreshAgents, selectionScope, send, stopActive]);
    return {
        model: {
            selectedProject,
            authStatus,
...
function updateAssistantMessage(state: Record<string, ChatMessage[]>, conversationKey: string, requestId: string, update: (message: ChatMessage) => ChatMessage) {
    return {
        ...state,
        [conversationKey]: (state[conversationKey] ?? []).map((message) => message.id === `assistant:${requestId}` ? update(message) : message)
    };
}
