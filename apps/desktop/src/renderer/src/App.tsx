import { tr } from "./i18n";
import { CircleAlert, FileText, FolderOpen, GitBranch, Globe2, LoaderCircle, Play, Plug, RefreshCw, Save, ShieldCheck, Square, SquareTerminal, Trash2, X } from "lucide-react";
import { useCallback, useEffect, useMemo, useRef, useState, type CSSProperties, type PointerEvent as ReactPointerEvent } from "react";
import type { AttachedBrowserState, AttachedBrowserTarget, ChatModel, DesktopChatAttachment, DesktopWorkflowDraft, DesktopWorkflowDraftSummary, DesktopWorkflowNodeRegistry, DesktopWorkflowRun, DesktopWorkflowRunEvent, ManagedProcessSummary, ManagedBrowserState, LocalApiGatewayState, LocalProjectChatMessage, LocalProjectChatSummary, LocalTriggerSummary, LocalTriggerKind, LocalSkillInstallReceipt, McpServerSummary, NativeAppConnectorSummary, ProjectChatEvent, ProjectChatRequest, WebSearchMode, ProjectContext, ProjectArtifactPreview, ProjectFileVersion, ProjectFileVersionSummary, ProjectFileTree, ProjectSearchResult, ProjectSummary, ReadResult, RouteMarketWorkApi, WorkState } from "../../shared/desktop-api";
...
import { SettingsPage, type SettingsView } from "./features/settings/SettingsPage";
...
import { projectFolderAvailable, projectFolderMessage, projectFolderStatus } from "./features/projects/project-folder-status";
import { WorkflowPage } from "./features/workflow/WorkflowPage";
import { connectWorkflowDraftNodes, duplicateWorkflowDraftNodes, layoutWorkflowDraft, moveWorkflowDraftNodes, removeWorkflowDraftEdges, removeWorkflowDraftNodes } from "./features/workflow/workflow-draft-graph";
import { createWorkflowDraftHistory, recordWorkflowDraftHistory, redoWorkflowDraftHistory, undoWorkflowDraftHistory } from "./features/workflow/workflow-draft-history";
...
type WorkspaceView = "chat" | "files" | "changes" | "versions" | "terminal" | "approvals" | "browser" | "mcp" | "workflow" | "agent" | "settings";
type WorkbenchPanel = "files" | "terminal" | "browser" | null;
const previewState: WorkState = {
    ...
    account: {
        ...
        spaces: [
            ...
            { id: "team_preview", name: "Good Team GGGGGGGGG!", kind: "team", teamId: "team_preview", avatarUrl: null, role: "owner" }
        ],
        membership: {
            planCode: "pro",
...
function getPreviewAgents() { return [
    {
...
const previewApi: RouteMarketWorkApi = {
    ...
    async updatePreferences(patch) { return patch; },
    async setLocale() { },
    async executeMenuCommand() { },
    async setTitleBarTheme() { },
    async setWorkbenchExpanded(expanded) { return { expanded, addedWidth: 0 }; },
    async getState() {
    ...
    async listMarketplaceCatalog() { return { schemaVersion: 1, revision: `sha256:${"0".repeat(64)}`, items: [] }; },
    async listMarketplacePluginInstallations() { return []; },
    async prepareMarketplacePluginInstall() { return desktopBridgeUnavailable(); },
    async cancelMarketplacePluginInstall() { return false; },
    async installMarketplacePlugin() { return desktopBridgeUnavailable(); },
    async setMarketplacePluginEnabled() { return desktopBridgeUnavailable(); },
    async removeMarketplacePlugin() { return desktopBridgeUnavailable(); },
    ...
    async removeLocalDataScope() { return false; },
    async showLocalData() { },
    ...
    async renameProject(localProjectId, displayName) {
        ...
        previewCurrentState = { ...previewCurrentState, projects: previewCurrentState.projects.map((item) => item.localProjectId === localProjectId ? renamed : item) };
        return renamed;
    },
    ...
    async removeProjectFolder(localProjectId, folderId) {
        ...
        if (!project)
            throw new Error("Project not found");
        const folders = (project.folders ?? []).filter((folder) => folder.folderId !== folderId)
            .map((folder, index) => ({ ...folder, primary: index === 0 }));
        const updated = { ...project, folders, hasFolder: folders.length > 0, folderStatus: folders.length > 0 ? "available" as const : "unlinked" as const, rootFingerprint: folders[0]?.folderId ?? "" };
        previewCurrentState = { ...previewCurrentState, projects: previewCurrentState.projects.map((item) => item.localProjectId === localProjectId ? updated : item) };
        return updated;
    },
    async openProjectFolder() { return true; },
    ...
    async chooseAndInstallProjectSkill(localProjectId) {
        ...
        previewProjectSkills = [
            ...previewProjectSkills.filter((skill) => skill.skillId !== installed.skillId),
            installed
        ];
        return installed;
    },
    ...
    async getWorkflowNodeRegistry() {
        const definitions = [
            { executorKey: "local.fs.read", title: tr("ui.dc995cddfa91"), description: tr("ui.5bff191f3e7c"), portability: "portable" as const },
            { executorKey: "local.browser.navigate", title: tr("ui.22d040b33dbe"), description: tr("ui.58d8f1ad8243"), portability: "device_bound" as const },
            { executorKey: "local.browser.product_extract", title: tr("ui.7a1ef038a396"), description: tr("ui.9924801cad0d"), portability: "device_bound" as const },
            { executorKey: "local.data.csv_export", title: tr("ui.d550b7bc237a"), description: tr("ui.1092ba3bb83f"), portability: "device_bound" as const },
            { executorKey: "local.app.vscode.open", title: "Visual Studio Code", description: tr("ui.6c9b42d40961"), portability: "requires_connector" as const }
        ].map((item) => ({
            ...item,
            definitionVersion: 1,
    ...
    async closeBrowserPage(localProjectId, pageId) {
        ...
        if (!pages.length)
            return previewApi.createBrowserPage(localProjectId);
    ...
    async updateBrowserProfile(_localProjectId, profileId, input) {
        previewBrowserState = {
            ...previewBrowserState,
            profiles: previewBrowserState.profiles.map((profile) => profile.profileId === profileId ? { ...profile, ...input } : profile)
        };
        return previewBrowserState;
    },
    ...
    async navigateBrowser(_localProjectId, url) {
        ...
        previewBrowserState = {
            ...
            url: normalizedUrl,
            title: "Preview page",
            pages: previewBrowserState.pages.map((page) => page.pageId === previewBrowserState.activePageId
    ...
    async navigateAttachedBrowser(url) {
        if (!previewAttachedBrowserState.target)
            throw new Error(tr("ui.df4d48a4010b"));
        previewAttachedBrowserState = {
            ...previewAttachedBrowserState,
    ...
    async fireLocalTrigger(triggerId) {
        ...
        if (!trigger)
            throw new Error("Local trigger not found");
        trigger.lastFiredAt = new Date().toISOString();
        return trigger;
    },
    async listNativeAppConnectors() {
        return [
            { connectorId: "vscode" as const, name: "Visual Studio Code", description: tr("ui.b58f1539b978"), available: true, executablePath: "C:/Preview/Code.exe", supportedExtensions: [] },
            { connectorId: "excel" as const, name: "Microsoft Excel", description: tr("ui.e1357050b778"), available: false, executablePath: null, supportedExtensions: [".xlsx", ".xls", ".csv"] },
            { connectorId: "powerpoint" as const, name: "Microsoft PowerPoint", description: tr("ui.572adf180c4f"), available: false, executablePath: null, supportedExtensions: [".pptx", ".ppt"] }
        ];
    },
    ...
    async deleteDesktopWorkflowDraft(_localProjectId, workflowId) { if (previewWorkflowDraft?.workflowId === workflowId)
        previewWorkflowDraft = null; },
    async runDesktopWorkflow(localProjectId, workflowId, input = {}) {
        if (!previewWorkflowDraft ||
    ...
    async listDesktopWorkflowRuns(localProjectId, workflowId) {
        return previewWorkflowRuns
            .filter((run) => run.localProjectId === localProjectId &&
    ...
    async saveModelProvider(input) { return { id: input.id ?? "provider_preview", name: input.name, protocol: input.protocol, compatibility: input.compatibility ?? "standard", baseUrl: input.baseUrl, headers: input.headers ?? [], hasApiKey: Boolean(input.apiKey), enabled: input.enabled, modelCount: input.models?.length ?? 0, models: input.models ?? [], lastSyncedAt: null, lastError: null }; },
    async syncModelProvider() { throw new Error("Preview mode"); },
    async removeModelProvider() { return false; },
    async getLocalApiGateway() { return structuredClone(previewLocalApiState); },
    ...
    async saveLocalApiGatewayRoute() { return previewApi.getLocalApiGateway(); },
    async removeLocalApiGatewayRoute() { return previewApi.getLocalApiGateway(); },
    async listLocalApiGatewayUsage() { return []; },
    async getLocalProjectChat(localProjectId, sessionId) {
        const chat = previewChats.find((item) => item.localProjectId === localProjectId && (!sessionId || item.sessionId === sessionId));
        return chat ? { sessionId: chat.sessionId, localProjectId: chat.localProjectId, messages: previewMessages.get(chat.sessionId) ?? [] } : null;
    },
    async listLocalProjectChats(localProjectId) { return previewChats.filter((chat) => chat.localProjectId === localProjectId); },
    async listRecentLocalChats(limit = 15) { return previewChats.slice(0, limit); },
    async createLocalProjectChat(localProjectId) { const now = new Date().toISOString(); const chat = { sessionId: `local_chat_${Date.now()}`, localProjectId, title: tr("chat.agent.none"), createdAt: now, updatedAt: now }; previewChats = [chat, ...previewChats]; previewMessages.set(chat.sessionId, []); return chat; },
    async renameLocalProjectChat(localProjectId, sessionId, title) { const current = previewChats.find((chat) => chat.localProjectId === localProjectId && chat.sessionId === sessionId); if (!current) throw new Error("Conversation not found"); const chat = { ...current, title, updatedAt: new Date().toISOString() }; previewChats = previewChats.map((item) => item.sessionId === sessionId ? chat : item); return chat; },
    async deleteLocalProjectChat(_localProjectId, sessionId) { previewChats = previewChats.filter((chat) => chat.sessionId !== sessionId); previewMessages.delete(sessionId); },
    async moveLocalProjectChat(_localProjectId, sessionId, targetProjectId) { const current = previewChats.find((chat) => chat.sessionId === sessionId); if (!current) throw new Error("Conversation not found"); const chat = { ...current, localProjectId: targetProjectId, updatedAt: new Date().toISOString() }; previewChats = [chat, ...previewChats.filter((item) => item.sessionId !== sessionId)]; return chat; },
...
export function App() {
    ...
    const selectedProject = useMemo(() => state.projects.find((project) => project.localProjectId === selectedProjectId) ?? null, [selectedProjectId, state.projects]);
    const selectedChatModel = useMemo(() => models.find((model) => model.code === selectedModelCode) ?? null, [models, selectedModelCode]);
    ...
    const chatMessages = selectedSessionId
        ? chatMessagesByProject[selectedSessionId] ?? []
        : [];
    const recentChatSourcePaths = useMemo(() => {
        const paths: string[] = [];
    ...
    const agentVersionKey = selectedChatAgent
        ? `${selectedProjectId ?? "general"}:${selectedChatAgent.id}`
        : null;
    const conversationAgentVersion = useMemo(() => resolveConversationAgentVersion(chatMessages, selectedChatAgent, agentVersionKey ? adoptedAgentRevisions[agentVersionKey] : undefined), [
        adoptedAgentRevisions,
        agentVersionKey,
        chatMessages,
        selectedChatAgent
    ]);
    const chatAgentSkills = useMemo(() => resolveDesktopAgentSkillAvailability(selectedChatAgent?.skills ?? [], projectContext, {
    ...
    useEffect(() => {
        let active = true;
        void Promise.all([
            Promise.all(state.projects.map(async (project) => [
                project.localProjectId,
                await api.listLocalProjectChats(project.localProjectId)
            ] as const)),
    ...
    useEffect(() => {
        ...
        if (availableMode !== webSearchMode)
            setWebSearchMode(availableMode);
    ...
    useEffect(() => {
        ...
        let active = true;
        void api.listLocalProjectChats(chatScope).then((chats) => {
            if (!active) return null;
            if (chatScope) setProjectChats((current) => ({ ...current, [chatScope]: chats }));
            const nextSessionId = selectedSessionId && chats.some((chat) => chat.sessionId === selectedSessionId)
                ? selectedSessionId : chatScope ? chats[0]?.sessionId ?? null : null;
            ...
            setChatMessagesByProject((current) => {
                ...
                if (activeRequestRef.current?.sessionId === chat.sessionId ||
            ...
            .catch((nextError) => {
            if (active)
                setError(nextError instanceof Error ? nextError.message : tr("ui.07b2f6b8bef1"));
        });
        return () => { active = false; };
    ...
    const selectedProcess = processes.find((process) => process.processId === selectedProcessId) ??
        processes[0] ?? null;
    const visibleApprovals = state.approvals.filter((approval) => !approval.projectId || approval.projectId === selectedProjectId);
    ...
    const visibleWorkflowDefinitions = useMemo(() => {
        ...
        return query
            ? definitions.filter((definition) => `${definition.title} ${definition.executorKey} ${definition.description}`
                .toLocaleLowerCase()
                .includes(query))
            : definitions;
    ...
    const refreshState = useCallback(async () => {
        ...
        if (authTransitionRef.current || requestEpoch !== stateRefreshEpochRef.current)
            return nextState;
        setState(nextState);
        setConnectionError(null);
        setStateLoaded(true);
        setSelectedProjectId((current) => current === null
            ? null
    ...
    useEffect(() => {
        const unsubscribe = api.onProjectChatEvent((event) => {
            ...
            if (!activeRequest || event.requestId !== activeRequest.requestId)
                return;
            const sessionId = activeRequest.sessionId;
            if (event.type === "error") {
                setChatMessagesByProject((current) => updateAssistantMessage(current, sessionId, event.requestId, (message) => ({
                    ...message,
    ...
    useEffect(() => {
        const listener = (event: Event) => {
            ...
            if (typeof relativePath === "string" && relativePath)
                void openChatArtifact(relativePath);
        };
    ...
    useEffect(() => {
        ...
        const next = remembered && models.some((model) => model.code === remembered)
            ? remembered
            : preferred && models.some((model) => model.code === preferred)
                ? preferred
                : null;
        if (next)
            setSelectedModelCode(next);
    }, [models, projectContext, selectedProjectId]);
    function selectChatModel(modelCode: string) {
        setSelectedModelCode(modelCode);
        if (selectedProjectId)
            writeProjectModelPreference(selectedProjectId, modelCode);
    }
    useEffect(() => {
        setSelectedProjectSkillId((current) => current && projectContext?.skills.some((skill) => skill.id === current) ? current : "");
    }, [projectContext]);
    useEffect(() => {
        ...
        const timer = window.setTimeout(() => {
            void api.searchProject(selectedProjectId, query)
                ...
                .catch((nextError) => {
                if (active)
                    setError(nextError instanceof Error ? nextError.message : tr("ui.f72d26a7c6e3"));
            })
                .finally(() => {
                if (active)
                    setSearching(false);
    ...
    const refreshProcesses = useCallback(async () => {
        ...
        setSelectedProcessId((current) => current && projectItems.some((item) => item.processId === current)
            ? current
            : projectItems[0]?.processId ?? null);
    }, [selectedProjectId]);
    useEffect(() => {
        let active = true;
        const expanded = Boolean(workbenchPanel && workspaceView === "chat");
        void api.setWorkbenchExpanded(expanded, 720)
            .then((result) => {
            if (!active)
                return;
    ...
    useEffect(() => {
        if (workbenchPanel !== "browser" || browserMode !== "managed" || !selectedProjectId) {
        ...
        const syncBounds = () => {
            ...
            void api.showBrowser(selectedProjectId, bounds).then((state) => {
                ...
                if (active)
                    setError(nextError instanceof Error ? nextError.message : tr("ui.cb4d258ff4ec"));
            });
        };
    ...
    }, [browserMode, browserScreenshot, selectedProjectId, workbenchPanel]);
    const refreshMcpServers = useCallback(async () => {
        const servers = await api.listMcpServers();
        setMcpServers(servers);
        setSelectedMcpServerId((current) => current && servers.some((server) => server.serverId === current)
            ? current
            : servers[0]?.serverId ?? null);
    }, []);
    useEffect(() => {
        if (workspaceView !== "mcp")
            return;
    ...
    useEffect(() => {
        if (workspaceView !== "workflow" || !selectedProjectId)
            return;
        let active = true;
        void api.getWorkflowNodeRegistry(selectedProjectId)
            ...
            .catch((nextError) => {
            if (active)
                setError(nextError instanceof Error ? nextError.message : tr("ui.aa882d2a9e8e"));
        });
        return () => { active = false; };
    ...
    useEffect(() => {
        if (workspaceView !== "workflow" || workflowPanel !== "triggers")
            return;
    ...
    useEffect(() => {
        if (workspaceView !== "workflow" || workflowPanel !== "connectors")
            return;
    ...
    useEffect(() => {
        ...
        void api.listDesktopWorkflowDrafts(selectedProjectId)
            ...
            .catch((nextError) => {
            if (active)
                setError(nextError instanceof Error ? nextError.message : tr("ui.58ae91ecd6a0"));
        })
            .finally(() => { if (active)
    ...
    useEffect(() => {
        if (workspaceView !== "workflow" || workflowPanel !== "canvas")
            return;
        const handleHistoryShortcut = (event: KeyboardEvent) => {
            ...
            if (key === "z" && event.shiftKey) {
                if (!workflowDraftHistory.future.length)
                    return;
                event.preventDefault();
                redoWorkflowDraft();
            }
            else if (key === "z") {
                if (!workflowDraftHistory.past.length)
                    return;
                event.preventDefault();
                undoWorkflowDraft();
            }
    ...
    useEffect(() => {
        ...
        void api.listChatModels()
            ...
            setSelectedModelCode((current) => nextModels.some((model) => model.code === current)
                ? current
                : nextModels[0]?.code ?? "");
        })
            .catch((nextError) => {
    ...
    async function switchSpace(spaceId: string): Promise<boolean> {
        if (spaceId === state.account?.activeSpaceId)
            return true;
        if (activeRequestId)
            await api.stopProjectMessage(activeRequestId).catch(() => undefined);
        await agentWorkspace.stopActive().catch(() => undefined);
    ...
    function upsertRecentChat(chat: LocalProjectChatSummary) {
        setRecentChats((current) => [chat, ...current.filter((item) => item.sessionId !== chat.sessionId)]
    ...
    async function deleteProjectChat(localProjectId: string | null, sessionId: string) {
        ...
        try {
            ...
            const nextChats = localProjectId ? (projectChats[localProjectId] ?? []).filter((item) => item.sessionId !== sessionId) : [];
    ...
    async function moveProjectChat(localProjectId: string | null, sessionId: string, targetProjectId: string | null) {
        ...
        try {
            ...
            setProjectChats((current) => {
                const next = { ...current };
                if (localProjectId) next[localProjectId] = (next[localProjectId] ?? []).filter((item) => item.sessionId !== sessionId);
                if (targetProjectId) next[targetProjectId] = [moved, ...(next[targetProjectId] ?? []).filter((item) => item.sessionId !== sessionId)];
                return next;
            });
    ...
    async function createProject(displayName: string, attachFolder: boolean) {
        ...
        try {
            ...
            if (attachFolder)
                await api.attachProjectFolder(project.localProjectId);
            await refreshState();
        }
        catch (nextError) {
    ...
    async function deleteProject(localProjectId: string) {
        setError(null);
        try {
            if (!await api.deleteProject(localProjectId))
                return;
    ...
    async function readProjectFile(relativePath: string, selectedSheetId?: string, pageNumber?: number) {
        ...
        try {
            if (isPreviewableArtifact(relativePath)) {
                const preview = await api.previewProjectArtifact(selectedProject.localProjectId, relativePath, selectedSheetId, pageNumber);
    ...
    function selectProjectFile(relativePath: string) {
        if (readResult &&
            ...
            !window.confirm(tr("ui.eec4753b4f44"))) {
            return;
        }
    ...
    async function saveProjectFile() {
        if (!selectedProject || !selectedFilePath || !readResult || savingFile)
            return;
        setSavingFile(true);
        setError(null);
        try {
    ...
    async function openFileVersions() {
        if (!selectedProjectId || !selectedFilePath || !readResult || newFileDraft)
            return;
    ...
    async function selectFileVersion(versionId: string) {
        if (!selectedProjectId || !selectedFilePath || fileVersionBusy)
            return;
        setFileVersionBusy(true);
        setError(null);
        try {
    ...
    async function restoreFileVersion() {
        if (!selectedProjectId || !selectedFilePath || !selectedFileVersion || fileVersionBusy)
            return;
        setFileVersionBusy(true);
        setError(null);
        try {
            const result = await api.restoreProjectFileVersion(selectedProjectId, selectedFilePath, selectedFileVersion.versionId);
            setReadResult(result);
            setFileDraft(result.text);
            setNewFileDraft(false);
            setWorkspaceView("chat");
    ...
    async function startProjectProcess() {
        if (!selectedProject || processBusy)
            return;
        setProcessBusy(true);
        setError(null);
        try {
    ...
    async function stopProjectProcess() {
        if (!selectedProcess || selectedProcess.status !== "running" || processBusy)
            return;
        setProcessBusy(true);
        setError(null);
        try {
    ...
    async function navigateCurrentBrowser() {
        if (browserBusy || (browserMode === "managed" && !selectedProjectId))
            return;
        setBrowserBusy(true);
        setError(null);
        try {
            if (browserMode === "attached") {
                ...
                if (state.target)
                    setBrowserAddress(state.target.url);
            }
            else {
    ...
    async function discoverAttachedTargets() {
        ...
        try {
            ...
            setSelectedAttachedTargetId((current) => current && targets.some((target) => target.targetId === current)
                ? current
                : targets[0]?.targetId ?? "");
        }
        catch (nextError) {
    ...
    async function toggleBrowserTakeover() {
        if (!browserState || !selectedProjectId)
            return;
        try {
    ...
    async function captureBrowserScreenshot() {
        if (browserMode === "managed" && !selectedProjectId)
            return;
        setBrowserBusy(true);
        setError(null);
        try {
    ...
    async function retryManagedBrowserOperation(operationId: string) {
        if (!selectedProjectId || browserBusy)
            return;
        setBrowserBusy(true);
        setError(null);
        try {
    ...
    async function selectManagedBrowserPage(pageId: string) {
        if (!selectedProjectId || browserBusy || pageId === browserState?.activePageId)
            return;
        setBrowserBusy(true);
        setError(null);
        try {
    ...
    async function updateManagedBrowserProfile(profileId: string, input: Parameters<RouteMarketWorkApi["updateBrowserProfile"]>[2]) {
        if (!selectedProjectId || browserBusy)
            return;
        setBrowserBusy(true);
        setError(null);
    ...
    async function createLocalTrigger() {
        if (!selectedProjectId || !triggerName.trim() || triggerBusy)
            return;
        setTriggerBusy(true);
        setError(null);
        try {
    ...
    async function openNativeConnector(connector: NativeAppConnectorSummary) {
        if (!selectedProjectId || !connector.available || connectorBusyId)
            return;
    ...
    function undoWorkflowDraft() {
        if (!workflowDraft || workflowDraftBusy)
            return;
    ...
    function duplicateWorkflowNodes(nodeIds: string[]): string[] {
        if (!workflowDraft)
            return [];
        const duplicated = duplicateWorkflowDraftNodes(workflowDraft, nodeIds, (kind) => `${kind}_${crypto.randomUUID().replaceAll("-", "")}`);
    ...
    function moveWorkflowNodes(positions: Array<{
        nodeId: string;
        x: number;
        y: number;
    }>) {
    ...
    function createWorkflowFromSkill(skillId: string, values: Record<string, string>) {
        if (!selectedProjectId || !workflowRegistry)
            return;
        if (workflowDraftDirty &&
            !window.confirm(tr("ui.a1c01065cd7d"))) {
            return;
        }
        try {
            setWorkflowDraft(workflowSkillById(skillId).createDraft({
                localProjectId: selectedProjectId,
    ...
    function connectWorkflowNodes(sourceNodeId: string, targetNodeId: string, sourcePortId?: string, targetPortId?: string) {
        if (!workflowDraft)
            return;
        const next = connectWorkflowDraftNodes(workflowDraft, sourceNodeId, targetNodeId, `edge_${crypto.randomUUID().replaceAll("-", "")}`, sourcePortId, targetPortId);
        if (next !== workflowDraft)
            updateWorkflowDraft(() => next);
    }
    ...
    async function deleteWorkflowDraft() {
        if (!selectedProjectId || !workflowDraft || workflowDraftBusy)
            return;
        setWorkflowDraftBusy(true);
        setError(null);
        try {
            ...
            const next = summaries[0]
                ...
                : null;
            const now = new Date().toISOString();
            setWorkflowDraft(next ?? { workflowId: `workflow_${crypto.randomUUID().replaceAll("-", "")}`, localProjectId: selectedProjectId, kind: "workflow", name: tr("ui.764827043646"), nodes: [], edges: [], createdAt: now, updatedAt: now });
            setWorkflowDraftHistory(createWorkflowDraftHistory());
            setWorkflowDraftDirty(false);
        }
        catch (nextError) {
    ...
    async function runWorkflowDraft() {
        ...
        try {
            ...
            const run = await api.runDesktopWorkflow(selectedProjectId, workflowDraft.workflowId, input as Record<string, unknown>);
    ...
    async function cancelWorkflowRun() {
        if (!selectedWorkflowRun || workflowRunBusy)
            return;
        setWorkflowRunBusy(true);
        setError(null);
        try {
    ...
    async function selectWorkflowDraft(workflowId: string) {
        if (!selectedProjectId || workflowDraftBusy || workflowId === workflowDraft?.workflowId)
            return;
        if (workflowDraftDirty && !window.confirm(tr("ui.83d78f6a8f78")))
            return;
        setWorkflowDraftBusy(true);
        setError(null);
        try {
    ...
    function createWorkflowDraft(kind: DesktopWorkflowDraft["kind"]) {
        if (!selectedProjectId)
            return;
        if (workflowDraftDirty && !window.confirm(tr("ui.1e44d6b7c346")))
            return;
        const now = new Date().toISOString();
        setWorkflowDraft({
    ...
    async function installLocalMcp() {
        if (!mcpName.trim() || !mcpCommand.trim() || mcpBusy)
            return;
        setMcpBusy(true);
        setError(null);
        try {
    ...
    async function removeLocalMcp() {
        if (!selectedMcpServer || mcpBusy)
            return;
        if (!window.confirm(tr("ui.ee7444866a65", [selectedMcpServer.name])))
            return;
        setMcpBusy(true);
        setError(null);
        try {
    ...
    async function callLocalMcpTool() {
        if (!selectedMcpServer || !selectedMcpTool || mcpBusy)
            return;
        setMcpBusy(true);
        setError(null);
        try {
            ...
            const result = await api.callMcpTool(selectedMcpServer.serverId, selectedMcpTool.name, parsed as Record<string, unknown>);
    ...
    async function sendMessage(messageOverride?: string, attachmentOverride?: DesktopChatAttachment[]) {
        ...
        if (editingMessageId) {
            try {
                requestMessages = messagesBeforeEditedUser(requestMessages, editingMessageId);
            }
            catch {
                setEditingMessageId(null);
            ...
            try {
                await api.truncateLocalProjectChat(selectedProjectId, editingMessageId);
            }
            catch (nextError) {
                setError(nextError instanceof Error
                    ? nextError.message
            ...
            setChatMessagesByProject((current) => ({
                ...current,
                [sessionId]: requestMessages
            }));
            setEditingMessageId(null);
        }
        const activeAgentVersion = resolveConversationAgentVersion(requestMessages, selectedAgent, agentVersionKey ? adoptedAgentRevisions[agentVersionKey] : undefined);
        ...
        if (selectedProject) {
            setProjectChats((current) => ({
                ...current,
                [selectedProject.localProjectId]: (current[selectedProject.localProjectId] ?? []).map((chat) => chat.sessionId === sessionId ? { ...chat, title: message.trim().slice(0, 80) || chat.title, updatedAt: sentAt } : chat)
            }));
        }
        setRecentChats((current) => current.map((chat) => chat.sessionId === sessionId
        ...
        try {
            await api.sendProjectMessage({
                ...
                ...(selectedChatModel?.preferredChatProtocol === "openai_responses" && selectedChatModel.supportsReasoningSummary
                ...
                ...(selectedAgent && activeAgentVersion ? { agent: {
                    agentId: selectedAgent.id,
                    agentRevision: activeAgentVersion.activeRevision,
                    executionEnvironment,
                    agentName: activeAgentVersion.name,
            ...
            setChatMessagesByProject((current) => updateAssistantMessage(current, sessionId, requestId, (assistant) => ({
                ...assistant,
    ...
    async function chooseChatAttachments() {
        ...
        setError(null);
        try {
            const selected = await api.chooseChatAttachments(Math.max(0, 5 - pendingChatAttachments.length));
            if (!selected.length)
                return;
    ...
    function renderFilesWorkbench() {
        return <FilesPage model={{
            selectedProject, projectFiles, treeLoading, searchQuery, searchResult, searching,
            selectedFilePath, readResult, assetPreview, fileDraft, loading, savingFile,
            fileVersionBusy, newFileDraft, hasFileChanges, error
        }} actions={{
            onChooseProject: () => void chooseProject(),
            ...
            onSelectSheet: (sheetId) => { if (selectedFilePath) void readProjectFile(selectedFilePath, sheetId); },
            onSelectPdfPage: (pageNumber) => { if (selectedFilePath) void readProjectFile(selectedFilePath, undefined, pageNumber); },
    ...
    function renderTerminalWorkbench() {
        return <section className="terminal-pane">
          <div className="terminal-toolbar"><div className="terminal-command"><span>$</span>
            <input value={processCommand} placeholder={tr("ui.f1dfc3dce904")} aria-label={tr("ui.64e5f7ae3d63")} disabled={!selectedProject || processBusy} onChange={(event) => setProcessCommand(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void startProjectProcess(); }}/>
            <button className="primary-button" type="button" disabled={!selectedProject || !processCommand.trim() || processBusy} onClick={() => void startProjectProcess()}>{processBusy ? <LoaderCircle className="spin" size={14}/> : <SquareTerminal size={14}/>} {tr("ui.0c3acd446f19")}</button>
          </div></div>
          <div className="terminal-layout">
            <div className="process-list">{processes.map((process) => <button key={process.processId} className={process.processId === selectedProcess?.processId ? "active" : ""} type="button" onClick={() => setSelectedProcessId(process.processId)}><span className={`process-status ${process.status}`}/><strong>{process.executable}</strong><small>{process.status}{process.exitCode === null ? "" : ` · ${process.exitCode}`}</small></button>)}{processes.length === 0 && <div className="process-empty">{tr("ui.3ae4096d7128")}</div>}</div>
            <div className="terminal-output"><div className="terminal-output-header"><span>{selectedProcess ? [selectedProcess.executable, ...selectedProcess.args].join(" ") : "Terminal"}</span><button className="secondary-button" type="button" disabled={!selectedProcess || selectedProcess.status !== "running" || processBusy} onClick={() => void stopProjectProcess()}><Square size={12} fill="currentColor"/>{tr("ui.a17f70a8d3d6")}</button></div><pre>{selectedProcess ? `${selectedProcess.stdout}${selectedProcess.stderr ? `\n[stderr]\n${selectedProcess.stderr}` : ""}` : tr("ui.db4e0ec0994f")}</pre>{selectedProcess?.outputTruncated && <span className="output-truncated">{tr("ui.0c76fca6a0ae")}</span>}</div>
          </div>
        </section>;
    }
    function renderBrowserWorkbench() {
        return <BrowserPage model={{
            localProjectId: selectedProjectId, mode: browserMode, state: browserState, address: browserAddress,
            busy: browserBusy, screenshot: browserScreenshot, attachedEndpoint, attachedTargets,
            selectedAttachedTargetId, attachedState, error
        }} actions={{
            onModeChange: (mode) => { setBrowserMode(mode); setBrowserScreenshot(null); },
            ...
            onDismissError: () => setError(null)
        }} viewportRef={browserViewportRef} addressRef={browserAddressRef}/>;
    }
    if (!stateLoaded || state.authStatus !== "signed_in") {
        return (<AuthGate api={api} loading={!stateLoaded} state={state} busy={authAction !== null} connectionError={connectionError ?? error} onSignIn={() => void signIn("login")} onRegister={() => void signIn("register")} onCancel={() => void signOut()}/>);
    }
    const selectedChatTitle = selectedSessionId
        ? recentChats.find((chat) => chat.sessionId === selectedSessionId)?.title ??
          (selectedProjectId ? projectChats[selectedProjectId]?.find((chat) => chat.sessionId === selectedSessionId)?.title : null)
        : null;
    const visibleWorkbenchPanel = workspaceView === "chat" ? workbenchPanel : null;
    const workbenchTitle = visibleWorkbenchPanel === "files"
        ? selectedFilePath ?? tr("workbench.files")
        : visibleWorkbenchPanel === "terminal"
            ? selectedProcess ? [selectedProcess.executable, ...selectedProcess.args].join(" ") : tr("workbench.terminal")
    ...
    return (<div className={`app-shell ${visibleWorkbenchPanel ? "has-workbench" : ""}`} style={visibleWorkbenchPanel ? { "--workbench-panel-width": `${workbenchPanelWidth}px` } as CSSProperties : undefined}>
      <AppTitleBar api={api} model={{ title: selectedChatTitle || "", canOpenProjectFolder: Boolean(selectedProject && selectedFolderAvailable) }} actions={{
            onNewChat: () => void createProjectChat(null),
            onNewProject: chooseProject,
            onOpenProjectFolder: () => { if (selectedProject) void openProjectFolder(selectedProject.localProjectId); },
            onToggleRail: () => window.dispatchEvent(new Event("routemarket:toggle-rail")),
            onOpenFiles: () => { if (selectedProject) { setWorkspaceView("chat"); setWorkbenchPanel("files"); } },
            onOpenTerminal: () => { if (selectedProject) { setWorkspaceView("chat"); setWorkbenchPanel("terminal"); } },
            onOpenBrowser: () => { if (selectedProject) { setWorkspaceView("chat"); setWorkbenchPanel("browser"); } },
            onOpenSettings: toggleSettings,
            onCheckUpdates: () => { void api.checkForUpdates(); }
        }}/>
      <AppRail activeView={workspaceView} state={state} selectedProjectId={selectedProjectId} selectedSessionId={selectedSessionId} projectChats={projectChats} recentChats={recentChats} authBusy={authAction !== null} onSelect={(view) => {
            if (view === "settings") {
                toggleSettings();
                return;
            }
            setWorkspaceView(view);
            if (view !== "chat") setWorkbenchPanel(null);
        }} onCreateProject={chooseProject} onCreateChat={(projectId) => void createProjectChat(projectId)} onSelectChat={selectProjectChat} onRenameChat={(projectId, sessionId, title) => void renameProjectChat(projectId, sessionId, title)} onMoveChat={(projectId, sessionId, targetProjectId) => void moveProjectChat(projectId, sessionId, targetProjectId)} onDeleteChat={(projectId, sessionId) => void deleteProjectChat(projectId, sessionId)} onSelectProject={(projectId) => {
            setSelectedProjectId(projectId);
            setSelectedSessionId(null);
            setWorkspaceView("chat");
            setError(null);
        }} onAttachProjectFolder={(projectId) => void attachProjectFolder(projectId)} onOpenProjectFolder={(projectId) => void openProjectFolder(projectId)} onEditProject={setEditingProjectId} onDeleteProject={(projectId) => void deleteProject(projectId)} onSignIn={() => void signIn()} onSignOut={() => void signOut()} onSwitchSpace={switchSpace} onUpgrade={() => void api.executeMenuCommand("openPlanUpgrade")} onTopUpCredits={() => void api.executeMenuCommand("openCreditsTopUp")} onOpenCreditsUsage={() => void api.executeMenuCommand("openCreditsUsage")} onOpenAccountCenter={() => void api.executeMenuCommand("openAccountCenter")}/>
      <main className={`workspace ${workspaceView === "settings" ? "workspace-app-view" : ""}`}>
        {workspaceView !== "settings" && <header className="workspace-header">
          <div className="header-actions">
            {selectedProject && !selectedFolderAvailable && (<button className="secondary-button" type="button" onClick={() => void attachProjectFolder(selectedProject.localProjectId)}>
                <FolderOpen size={15}/>
            ...
            <div className="workspace-tool-buttons" role="toolbar" aria-label={tr("workbench.tools")}>
              <OutputMenu activePanel={visibleWorkbenchPanel} contextKey={`${selectedProjectId ?? ""}:${selectedSessionId ?? ""}`} disabled={!selectedProject} localFilesDisabled={Boolean(selectedProject && !selectedFolderAvailable)} files={projectFiles} processes={processes} browserState={browserState} selectedFilePath={selectedFilePath} selectedProcessId={selectedProcessId} recentSourcePaths={recentChatSourcePaths} onOpen={() => { void refreshProcesses(); void refreshProjectFiles(); }} onRefreshProcesses={() => { void refreshProcesses(); }} onCreateFile={prepareNewProjectFile} onOpenPanel={(panel) => { setWorkspaceView("chat"); if (panel === "browser") setBrowserScreenshot(null); setWorkbenchPanel(panel); }} onOpenProcess={(processId) => { setSelectedProcessId(processId); setWorkspaceView("chat"); setWorkbenchPanel("terminal"); }} onOpenFile={(relativePath) => { setWorkspaceView("chat"); setWorkbenchPanel("files"); selectProjectFile(relativePath); }}/>
              <span className="workspace-tool-divider"/>
              <button className={workspaceView === "changes" ? "active" : ""} type="button" disabled={!hasFileChanges} title={tr("ui.9d83d547ff13")} onClick={reviewProjectFileChanges}><GitBranch size={16}/>{hasFileChanges && <span className="change-dot"/>}</button>
              <button className={workspaceView === "versions" ? "active" : ""} type="button" disabled={!readResult || newFileDraft} title={tr("ui.989d1affa089")} onClick={() => void openFileVersions()}><RefreshCw size={16}/></button>
              <button className={workspaceView === "approvals" ? "active" : ""} type="button" title={tr("ui.5ce60cb75d20")} onClick={() => setWorkspaceView("approvals")}><ShieldCheck size={16}/></button>
            </div>
            <ActivityMenu activities={state.activities} onClear={() => {
        ...
        <div className="workspace-body">
          {state.workerStatus === "offline" && (<div className="workspace-offline-banner" role="status">
              <CircleAlert size={15}/>
              <span>{tr("ui.cb0421fb2cb9")}</span>
              <button type="button" onClick={() => void refreshState().catch(handleConnectionError)}>
                <RefreshCw size={13}/>{tr("ui.bac95e1e52fe")}</button>
            </div>)}
          {workspaceView === "settings" ? (<SettingsPage dataApi={api} initialView={settingsInitialView} onProvidersChanged={() => setModelCatalogRevision((value) => value + 1)}/>) : workspaceView === "agent" ? (<AgentPage model={agentWorkspace.model} actions={agentWorkspace.actions}/>) : workspaceView === "workflow" ? (<WorkflowPage model={{
                panel: workflowPanel,
                registry: workflowRegistry ? { ...workflowRegistry, definitions: workflowRegistry.definitions.map(localizeWorkflowNodeDefinition) } : null,
                ...
                onDismissError: () => setError(null)
            }}/>) : workspaceView === "mcp" ? (<section className="mcp-pane">
              <div className="mcp-install-bar">
                <select value={mcpTransport} aria-label="MCP transport" onChange={(event) => {
                ...
                <input value={mcpName} placeholder={tr("ui.02448b8f9a51")} aria-label={tr("ui.83bfac99975a")} onChange={(event) => setMcpName(event.target.value)}/>
                <input value={mcpCommand} placeholder={mcpTransport === "stdio"
                ? tr("ui.49817b5eb39e") : tr("ui.570e85a9e905")} aria-label={mcpTransport === "stdio" ? tr("ui.1674401a8668") : "MCP Streamable HTTP URL"} onChange={(event) => setMcpCommand(event.target.value)} onKeyDown={(event) => {
                if (event.key === "Enter")
                    void installLocalMcp();
            }}/>
                <button className="primary-button" type="button" disabled={!mcpName.trim() || !mcpCommand.trim() || mcpBusy} onClick={() => void installLocalMcp()}>{mcpBusy ? <LoaderCircle className="spin" size={14}/> : <Plug size={14}/>}{tr("ui.087db63ab10b")}</button>
              </div>
              <div className="mcp-layout">
                <aside className="mcp-server-list">
                  {mcpServers.map((server) => (<button key={server.serverId} className={server.serverId === selectedMcpServer?.serverId ? "active" : ""} type="button" onClick={() => {
                    setSelectedMcpServerId(server.serverId);
                  ...
                  {mcpServers.length === 0 && (<div className="mcp-empty"><Plug size={24}/><span>{tr("ui.eb2a522dad83")}</span></div>)}
                </aside>
                <div className="mcp-detail">
                  {selectedMcpServer ? (<>
                    ...
                    : selectedMcpServer.url}</code>
                        ...
                        <div>
                          <button className="secondary-button" type="button" disabled={mcpBusy} onClick={() => void toggleLocalMcp()}>
                            {selectedMcpServer.status === "online"
                    ? <><Square size={12} fill="currentColor"/>{tr("ui.a17f70a8d3d6")}</>
                    : <><Play size={12} fill="currentColor"/>{tr("ui.ebd26da42171")}</>}
                          </button>
                          <button className="danger-icon-button" type="button" title={tr("ui.014e9125fe4d")} disabled={mcpBusy} onClick={() => void removeLocalMcp()}><Trash2 size={14}/></button>
                        </div>
                      </div>
                      {selectedMcpServer.lastError && (<div className="mcp-server-error">{selectedMcpServer.lastError}</div>)}
                      <div className="mcp-tools">
                        <div className="mcp-tool-list">
                          <span>TOOLS</span>
                          {selectedMcpServer.tools.map((tool) => (<button key={tool.name} className={tool.name === selectedMcpTool?.name ? "active" : ""} type="button" onClick={() => {
                        ...
                        setMcpResult("");
                          ...
                          {selectedMcpServer.status === "online" && selectedMcpServer.tools.length === 0 && (<div className="mcp-empty compact">{tr("ui.828f0dd16340")}</div>)}
                        </div>
                        <div className="mcp-tool-runner">
                          {selectedMcpTool ? (<>
                              <div>
                                <strong>{selectedMcpTool.name}</strong>
                                <button className="primary-button" type="button" disabled={mcpBusy || selectedMcpServer.status !== "online"} onClick={() => void callLocalMcpTool()}>{mcpBusy ? <LoaderCircle className="spin" size={13}/> : <Play size={13}/>}{tr("ui.b1fb011bbffa")}</button>
                              </div>
                              <label>{tr("ui.8af554439dff")}</label>
                              <textarea value={mcpToolArgs} spellCheck={false} onChange={(event) => setMcpToolArgs(event.target.value)}/>
                              <label>{tr("ui.50a3351c37b2")}</label>
                              <pre>{mcpResult || tr("ui.a39829fbe50d")}</pre>
                            </>) : (<div className="mcp-empty"><span>{tr("ui.dc2dfe354bac")}</span></div>)}
                        </div>
                      </div>
                    </>) : (<div className="mcp-empty"><Plug size={28}/><span>{tr("ui.70defbb628a3")}</span></div>)}
                </div>
              </div>
              {error && (<div className="error-banner" role="alert">
                  <CircleAlert size={18}/><span>{error}</span>
                  <button type="button" title={tr("ui.6c14bd7f6f9e")} onClick={() => setError(null)}><X size={14}/></button>
                </div>)}
            </section>) : workspaceView === "browser" ? (<BrowserPage model={{
                localProjectId: selectedProjectId,
                mode: browserMode,
                state: browserState,
                ...
                onToggleAttachedConnection: () => void toggleAttachedConnection(),
                onDismissError: () => setError(null)
            }} viewportRef={browserViewportRef} addressRef={browserAddressRef}/>) : workspaceView === "chat" ? (<div className="workspace-split"><div className="workspace-split-main"><ChatPage selectedProject={selectedProject} hasConversation={Boolean(selectedSessionId)} messages={chatMessages} activeRequestId={activeRequestId} includeFileContext={includeFileContext} selectedFilePath={selectedFilePath} readResult={readResult} draft={draft} attachments={pendingChatAttachments} authStatus={state.authStatus} models={models} selectedModelCode={selectedModelCode} executionEnvironment={executionEnvironment} webSearchMode={webSearchMode} modelsLoading={modelsLoading} agents={agentWorkspace.model.agents} agentsLoading={agentWorkspace.model.agentsLoading} selectedAgentId={agentWorkspace.model.selectedAgentId} selectedAgent={agentWorkspace.model.selectedAgent} agentVersion={conversationAgentVersion} agentSkills={chatAgentSkills} projectContext={projectContext} selectedProjectSkillId={selectedProjectSkillId} projectSkillActions={selectedProjectId && selectedFolderAvailable ? {
                localProjectId: selectedProjectId,
                list: () => api.listProjectSkills(selectedProjectId),
                listCloud: () => api.listDownloadableCloudSkills(),
                install: () => api.chooseAndInstallProjectSkill(selectedProjectId),
                installCloud: (skillId, versionId) => api.installCloudSkill(selectedProjectId, skillId, versionId),
                remove: (skillId) => api.removeInstalledProjectSkill(selectedProjectId, skillId),
                onChanged: async () => {
                    const context = await api.getProjectContext(selectedProjectId);
                    setProjectContext(context);
                }
            } : null} editingMessageId={editingMessageId} error={error} onAttachProjectFolder={() => {
                if (selectedProject)
                    void attachProjectFolder(selectedProject.localProjectId);
            }} onDraftChange={setDraft} onChooseAttachments={() => void chooseChatAttachments()} onRemoveAttachment={(attachmentId) => void removeChatAttachment(attachmentId)} onSend={() => void sendMessage()} onRetry={retryMessage} onEditMessage={(messageId) => {
                const message = chatMessages.find((candidate) => candidate.id === messageId && candidate.role === "user");
                if (!message)
                    return;
                ...
                setDraft("");
                setPendingChatAttachments([]);
            }} onStop={() => void stopMessage()} onModelChange={selectChatModel} onManageModelProviders={openModelProviders} onExecutionEnvironmentChange={setExecutionEnvironment} onWebSearchModeChange={setWebSearchMode} onAgentChange={(agentId) => {
                ...
                if (!agentVersionKey || !currentAgent)
                    return;
                setAdoptedAgentRevisions((current) => ({
                    ...current,
                    [agentVersionKey]: currentAgent.revision
                }));
            }} onProjectSkillChange={setSelectedProjectSkillId} onIncludeFileContextChange={setIncludeFileContext} onDismissError={() => setError(null)} onOpenArtifact={(relativePath) => void openChatArtifact(relativePath)}/></div></div>) : workspaceView === "approvals" ? (<ApprovalPage approvals={visibleApprovals} policies={visibleApprovalPolicies} projectName={selectedProject?.displayName ?? null} busyPolicyId={busyApprovalPolicyId} onRevokePolicy={(policyId) => void revokeApprovalPolicy(policyId)}/>) : workspaceView === "terminal" ? (<section className="terminal-pane">
              <div className="terminal-toolbar">
                <div className="terminal-command">
                  <span>$</span>
                  <input value={processCommand} placeholder={tr("ui.f1dfc3dce904")} aria-label={tr("ui.64e5f7ae3d63")} disabled={!selectedProject || processBusy} onChange={(event) => setProcessCommand(event.target.value)} onKeyDown={(event) => {
                if (event.key === "Enter")
                    void startProjectProcess();
            }}/>
                  <button className="primary-button" type="button" disabled={!selectedProject || !processCommand.trim() || processBusy} onClick={() => void startProjectProcess()}>
                    {processBusy ? <LoaderCircle className="spin" size={14}/> : <SquareTerminal size={14}/>}{tr("ui.0c3acd446f19")}</button>
                </div>
              </div>
              <div className="terminal-layout">
                <div className="process-list">
                  {processes.map((process) => (<button key={process.processId} className={process.processId === selectedProcess?.processId ? "active" : ""} type="button" onClick={() => setSelectedProcessId(process.processId)}>
                ...
                <div className="terminal-output">
                  <div className="terminal-output-header">
                    ...
                    <button className="secondary-button" type="button" disabled={!selectedProcess || selectedProcess.status !== "running" || processBusy} onClick={() => void stopProjectProcess()}><Square size={12} fill="currentColor"/>{tr("ui.a17f70a8d3d6")}</button>
                  </div>
                  <pre>{selectedProcess
                ? `${selectedProcess.stdout}${selectedProcess.stderr ? `\n[stderr]\n${selectedProcess.stderr}` : ""}`
                : tr("ui.db4e0ec0994f")}</pre>
            ...
            </section>) : workspaceView === "versions" && readResult ? (<section className="versions-pane">
              <div className="versions-header">
                <div><span className="eyebrow">Local Version History</span><h2>{tr("ui.0e3758fba9e9")}</h2><p>{selectedFilePath}</p></div>
                <div className="versions-actions">
                  <button className="secondary-button" type="button" onClick={() => { setWorkspaceView("chat"); setWorkbenchPanel("files"); }}>{tr("ui.40c16624c742")}</button>
                  <button className="secondary-button" type="button" disabled={!selectedFileVersion || fileVersionBusy} onClick={() => void exportSelectedProjectFile(selectedFileVersion?.versionId)}><FolderOpen size={13}/>{tr("ui.10dac709e474")}</button>
                  <button className="primary-button" type="button" disabled={!selectedFileVersion || fileVersionBusy} onClick={() => void restoreFileVersion()}>{fileVersionBusy ? <LoaderCircle className="spin" size={14}/> : <RefreshCw size={14}/>}{tr("ui.da516977ec7d")}</button>
                </div>
              </div>
              <div className="versions-layout">
                <div className="version-list">
                  ...
                  {fileVersions.length === 0 && <div className="version-empty"><RefreshCw size={22}/><span>{tr("ui.6a3983d72541")}</span></div>}
                </div>
                <div className="version-comparison">
                  {selectedFileVersion ? <DiffPreview before={selectedFileVersion.text} after={readResult.text}/> : <div className="version-empty"><GitBranch size={25}/><span>{tr("ui.7ccef183ec59")}</span></div>}
                </div>
              </div>
              {error && <div className="error-banner" role="alert"><CircleAlert size={18}/><span>{error}</span><button type="button" title={tr("ui.6c14bd7f6f9e")} onClick={() => setError(null)}><X size={14}/></button></div>}
            </section>) : workspaceView === "changes" && readResult ? (<section className="changes-pane">
              <div className="changes-header">
                ...
                <div className="changes-actions">
                  <button className="secondary-button" type="button" disabled={savingFile} onClick={() => { setWorkspaceView("chat"); setWorkbenchPanel("files"); }}>{tr("ui.40c16624c742")}</button>
                  <button className="primary-button" type="button" disabled={savingFile || !hasFileChanges} onClick={() => void saveProjectFile()}>
                    {savingFile
              ...
              {error && (<div className="error-banner" role="alert">
                ...
                onSelectSheet: (sheetId) => {
                    if (selectedFilePath)
                        void readProjectFile(selectedFilePath, sheetId);
                },
                onSelectPdfPage: (pageNumber) => {
                    if (selectedFilePath)
                        void readProjectFile(selectedFilePath, undefined, pageNumber);
                },
                onExportFile: () => void exportSelectedProjectFile(),
                onOpenVersions: () => void openFileVersions(),
                onReviewChanges: reviewProjectFileChanges,
      ...
      {visibleWorkbenchPanel && <aside className="workspace-side-panel"><div className="workspace-side-panel-resizer" role="separator" aria-orientation="vertical" onPointerDown={beginWorkbenchResize}/><header className="workspace-side-panel-header"><div className="workspace-side-panel-title">{visibleWorkbenchPanel === "files" ? <FileText size={15}/> : visibleWorkbenchPanel === "terminal" ? <SquareTerminal size={15}/> : <Globe2 size={15}/>}<strong title={workbenchTitle}>{workbenchTitle}</strong></div><button type="button" title={tr("workbench.close")} onClick={() => setWorkbenchPanel(null)}><X size={15}/></button></header><div className="workspace-side-panel-body">{visibleWorkbenchPanel === "files" ? renderFilesWorkbench() : visibleWorkbenchPanel === "terminal" ? renderTerminalWorkbench() : renderBrowserWorkbench()}</div></aside>}
      <ProjectCreateDialog open={projectDialogOpen} busy={projectActionBusy} onClose={() => { if (!projectActionBusy)
        setProjectDialogOpen(false); }} onCreate={(name, attachFolder) => void createProject(name, attachFolder)}/>
      <ProjectEditDialog project={editingProject} busy={projectActionBusy} onClose={() => { if (!projectActionBusy) setEditingProjectId(null); }} onSave={(name) => { if (editingProject) void renameProject(editingProject.localProjectId, name); }} onAttachFolder={() => { if (editingProject) void attachProjectFolder(editingProject.localProjectId); }} onRemoveFolder={(folderId) => { if (editingProject) void removeProjectFolder(editingProject.localProjectId, folderId); }} onRemove={() => { if (editingProject) { const projectId = editingProject.localProjectId; setEditingProjectId(null); void deleteProject(projectId); } }}/>
    </div>);
}
...
function updateAssistantMessage(state: Record<string, ChatMessage[]>, sessionId: string, requestId: string, update: (message: ChatMessage) => ChatMessage) {
    return {
        ...state,
        [sessionId]: (state[sessionId] ?? []).map((message) => message.id === `assistant:${requestId}` ? update(message) : message)
    };
}
function updateChatToolActivity(tools: NonNullable<ChatMessage["tools"]>, event: Extract<ProjectChatEvent, {
...
function fileVersionSourceLabel(source: ProjectFileVersionSummary["source"]): string {
    if (source === "created")
        return tr("ui.c2098a5e9c7c");
    if (source === "restored")
        return tr("ui.0c3b2436730f");
    if (source === "baseline")
        return tr("ui.3740b0381797");
